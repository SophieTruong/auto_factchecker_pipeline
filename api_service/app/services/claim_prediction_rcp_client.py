...
class ClaimPredictionRcpClient:
    ...
    async def get_model_predictions(self, claim_list: List[str]):
        try:
            ...
            logger.info(
                f"A correlation_id is generated for get_model_predictions: {correlation_id}"
            )
            ...
            message = Message(
                body=json.dumps(
                    {"claim": claim_list, "correlation_id": correlation_id}
                ).encode(),
            ...
            await self.channel.default_exchange.publish(
                message, routing_key="rpc_claim_detection_queue"
            )
