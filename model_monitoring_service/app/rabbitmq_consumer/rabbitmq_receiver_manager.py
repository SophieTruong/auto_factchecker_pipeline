...
class RabbitMqReceiverManager:
    ...
    async def connect(self):
        if not self.is_connected:
            ...
            # Perform connection
            self.connection = await connect_robust(RABBITMQ_URL, loop=asyncio.get_running_loop())
    ...
    async def process_message(self, processed_message):
        ...
        parsed_message = self.monitoring_service.parse_message_body(processed_message.body)
        ...
        if event_type == "complete" and module_name == "evidence_retrieval":
            ...
            success = await self.monitoring_service.handle_evidence_retrieval_metrics(self.mongodb_manager, parsed_message)
        elif event_type == "created" and module_name == "claim_annotation":
            print(f" [x] Processing claim annotation metrics: {parsed_message!r}")
            success = await self.monitoring_service.handle_claim_annotation_metrics(self.mongodb_manager, parsed_message)
        else:
