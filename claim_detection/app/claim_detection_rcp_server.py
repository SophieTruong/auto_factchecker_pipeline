...
async def handle_claim_request(message_body_dict: dict, db):
    ...
    try:
        ...
        if request_type == "claim_detection_insert":
            ...
            claim_predictions =  await claim_detection_service.get_predictions(source_document)
            ...
            # Handle get
            claims = await claim_detection_service.get_claims(data["start_date"], data["end_date"])
            ...
            claim_annotations = await claim_annotation_service.create_claim_annotation(BatchClaimAnnotationInput(**data))
            ...
            claim_annotations = [ClaimAnnotation(**claim_annotation) for claim_annotation in data]
            
            # Handle annotation update
            updated_claim_annotations = await claim_annotation_service.update_claim_annotation(claim_annotations)
            ...
            async for message in qiterator:
                ...
                try:
                    ...
                    async with message.process(requeue=False):
                        ...
                        logger.info(f" [.] Claim detection RCP server: message_body received and decoded")
                        
                        message_body_dict = json.loads(message_body)
                        
                        with get_db() as db:
                            
                            response_body = await handle_claim_request(message_body_dict, db=db)

                            response_body_data = serialize_response_data(response_body["data"])
                            
                            logger.info(f" [.] response_body_data: {response_body_data}")
